...
async def give_filter(client, message):
    if SUPPORT_CHAT_ID != message.chat.id and MASTER_CHAT_ID != message.chat.id:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 InlineKeyboardButton('Jᴏɪɴ Cʜᴀɴɴᴇʟ', url=f"https://t.me/+XIFvZdfRYt42YWY1"),
                 InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
        ...
        btn2 = [[
                 InlineKeyboardButton('Sᴇᴀʀᴄʜ 🔍', url=f"https://t.me/+AREvOAJMu6BiOTJl"),
                 InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
    ...
    # Thanks to @CoderluffyTG for fixing this 
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/UHDSupport")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
