...
async def give_filter(client, message):
    if message.chat.id == SUPPORT_CHAT_ID or message.chat.id == MASTER_CHAT_ID:
        ...
        button = [[
                       InlineKeyboardButton("✅ Bᴜʏ Sᴜʙsᴄʀɪᴘᴛɪᴏɴ ✅", url=f"https://t.me/ORGPrime/25")
        ]]
        if total_results == 0:
            return
        else:
            return await message.reply_text(text=(script.MINFO_TXT.format(message.from_user.mention)), reply_markup=InlineKeyboardMarkup(button))
